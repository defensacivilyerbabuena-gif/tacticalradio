...
import { Mic, Power, AlertTriangle, Wifi, Globe, Layers } from 'lucide-react';
...
export const RadioControl: React.FC<RadioControlProps> = ({
  ...
  return (
    <div className="flex flex-col h-full bg-gray-950 text-white p-3 md:p-6 select-none relative overflow-hidden">
        ...
        {/* Header Superior con Selector de Canales */}
        <div className="flex justify-between items-center z-10 mb-3 md:mb-6">
            <div className="flex items-center gap-2">
                <button 
                  onClick={isConnected ? onDisconnect : onConnect} 
                  className={`w-10 h-10 md:w-11 md:h-11 rounded-full flex items-center justify-center transition-all ${isConnected ? 'bg-emerald-600 shadow-lg shadow-emerald-900/40' : 'bg-red-900/20 border border-red-500/50 text-red-500'}`}
                >
                    <Power size={18} />
                </button>
                <div className="flex flex-col">
                  <span className="font-mono text-[9px] font-bold text-gray-500">COMMS_LINK</span>
                  <div className="flex bg-black/50 border border-white/10 rounded p-0.5 mt-0.5">
                    {['1', '2'].map((ch) => (
                      <button
                        key={ch}
                        onClick={() => onChannelChange(ch)}
                        className={`px-3 py-0.5 text-[10px] font-black font-mono transition-all rounded ${currentChannel === ch ? 'bg-orange-600 text-white' : 'text-gray-600 hover:text-gray-400'}`}
                      >
                        CH{ch}
                      </button>
                    ))}
        ...
        {/* Visor de Estado */}
        <div className="flex-1 flex flex-col items-center justify-center z-10 gap-3 md:gap-8 min-h-[250px]">
            <div className="w-full bg-black/60 border border-white/5 p-3 md:p-4 rounded font-mono shadow-inner min-h-[70px] flex flex-col justify-center">
                <div className="flex justify-between text-[7px] md:text-[9px] text-orange-500/40 mb-1">
                  <span className="uppercase tracking-widest">Digital Tactical Net</span>
                  <span className="flex items-center gap-1"><Layers size={8} /> CANAL_{currentChannel}</span>
                </div>
                {connectionState === ConnectionState.CONNECTING && <div className="text-orange-400 text-center animate-pulse text-xs uppercase tracking-widest">Sincronizando...</div>}
                {isConnected && !isTalking && !lastTranscript && <div className="text-emerald-500 text-center text-[10px] md:text-sm font-bold tracking-[0.2em] uppercase">Canal despejado</div>}
                {isTalking && <div className="text-orange-500 text-center font-black text-sm md:text-lg animate-pulse uppercase tracking-widest">Transmitiendo</div>}
            ...
            <div className="relative">
                ...
                >
                    ...
                    <span className="text-[8px] md:text-[10px] font-black tracking-[0.2em] mt-2 uppercase">{isTalking ? 'Hablando' : isConnected ? 'Push To Talk' : 'Sin Señal'}</span>
                </button>
            </div>
            ...
            <div className="grid grid-cols-2 gap-2 w-full mt-auto">
                <div className="bg-black/40 p-2 rounded border border-white/5 flex items-center gap-2">
                    ...
                    <span className="text-[8px] font-mono text-gray-500 uppercase">CH_{currentChannel}_LINK</span>
                </div>
                <div className="bg-black/40 p-2 rounded border border-white/5 flex items-center gap-2">
                    <Wifi size={12} className="text-emerald-500" />
                    <span className="text-[8px] font-mono text-gray-500 uppercase">TX_100%</span>
                </div>
            </div>
        </div>
    </div>
